...
class Fingerprint:
    def __init__(self, name:str, **attrs: Any) -> None:
        ...
        self.pricing: List[str] = self._prepareList(attrs['princing']) if 'princing' in attrs else []
        ...
        self.text: List[Pattern] = self._preparePattern(attrs['text']) if 'text' in attrs else []
    ...
    @classmethod
    def _prepareList(cls, thing: Any) -> List[Any]:
    ...
    def _preparePattern(cls, pattern: Union[str, List[str]]) -> List[Pattern]:
        ...
        if isinstance(pattern, list):
            for p in pattern:
                pattern_objects.extend(cls._preparePattern(p))
        else:
            attrs = {}
    ...
    @classmethod
    def _prepareDom(cls, thing: Union[str, List[str], 
                                    Dict[str, Dict[str, 
        ...
        if isinstance(thing, str):
            ...
            for cssselect, clause in thing.items():
                ...
                if clause.get('exists') is not None:
                    _exists = True
                if clause.get('text'):
                    _prep_text_patterns = cls._preparePattern(clause['text'])
                if clause.get('attributes'):
                    _prep_attr_patterns ={}
                    for _key, pattern in clause['attributes'].items(): #type: ignore
                        _prep_attr_patterns[_key] = cls._preparePattern(pattern)
